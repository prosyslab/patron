...
let match_bug out_dir donee donee_maps pattern =
  let status = Chc.pattern_match out_dir "donee" donee_maps donee pattern in
       ...
       ~f:(fun donor_name ->
         ...
         List.iter matched_donors ~f:(fun donor_cand ->
             ...
             if is_patched then (
               ...
               exit 0)))
       matched_donors *)
  let donor_cand = Filename.concat db_dir "test-1" in
  TF.transplant donor_cand donee_dir out_dir
