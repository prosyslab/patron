...
module Diff = struct
  ...
  let rec make_diff_exp code parent prev next (exp_lst : Cil.exp list) =
    ...
    | [] -> []
    | hd :: tl ->
        let context = { parent; left_sibling = prev; right_sibling = next } in
        if code = 0 then
  ...
  let extract_exp code parent prev exp1 exp2 expl1 expl2 =
    if code = update_code then (
      ...
      [
        UpdateExp
          ({ parent; left_sibling = Null; right_sibling = Null }, exp1, exp2);
      ])
    else
      ...
      if insertion <> [] then (
        ...
        if deletion <> [] then (
          ...
          [
            UpdateExp
              ({ parent; left_sibling = prev; right_sibling = next }, exp1, exp2);
          ]
  ...
  and fold_param2 parent prev el1 el2 =
    ...
    | [], hd :: tl ->
        let context =
          { parent; left_sibling = prev_node; right_sibling = Null }
        in
        let _ = L.debug "exp insertion detected" in
  ...
  let extract_call parent lv1 e1 el1 lv2 e2 el2 =
    let lval_diff =
      ...
      | None, Some lv ->
          ...
          [
            InsertLval
              ({ parent; left_sibling = Null; right_sibling = Null }, lv);
          ]
      | Some lv, None ->
      ...
      | Some l1, Some l2 ->
          if H.string_of_lval l1 = H.string_of_lval l2 then []
            ...
            [
              UpdateLval
                ({ parent; left_sibling = Null; right_sibling = Null }, l1, l2);
            ]
      | _ -> []
    in
    let exp_diff =
      if H.string_of_exp e1 = H.string_of_exp e2 then []
        ...
        [
          UpdateExp
            ({ parent; left_sibling = Null; right_sibling = Null }, e1, e2);
        ]
    in
  ...
  let extract_set parent lv1 e1 lv2 e2 =
    let context = { parent; left_sibling = Null; right_sibling = Null } in
    let lval_diff =
  ...
  let rec make_diff_glob code prev next glob_lst =
    ...
    | hd :: tl ->
        let context =
          { parent = Null; left_sibling = prev; right_sibling = next }
        in
        if code = 0 then
  ...
  and fold_globals2 prev doner_gobals patch_globals =
    ...
    | [], hd2 :: tl2 ->
        let context =
          { parent = Null; left_sibling = prev_node; right_sibling = Null }
        in
        let _ = L.debug "global insertion detected" in
