...
let transplant db_dir donee_dir patron_out_dir =
  ...
  let sink_node = H.extract_snk (Filename.concat donee_dir "sparrow-out/taint/datalog/IOErrorConstraint.rules") in
  let edit_function = S.parse_edit_function sink_node edit_function_path in
  let translated = T.translate donee_sparrow_dir donee edit_function solution_path in
