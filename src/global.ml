...
module Z3Env = struct
  ...
  let mk_fixedpoint z3ctx =
    ...
    Z3.Params.add_bool param
      ...
      true;
    Z3.Params.add_bool param (mk_string_symbol "print_answer") true;
  ...
  let mk_env () =
    ...
    let node =
      Z3.FiniteDomain.mk_sort_s z3ctx "node" (Int64.of_int !sort_size)
    in
    let ast_node =
      Z3.FiniteDomain.mk_sort_s z3ctx "ast_node" (Int64.of_int !sort_size)
    in
    let lval =
      Z3.FiniteDomain.mk_sort_s z3ctx "lval" (Int64.of_int !sort_size)
    in
    let expr =
      Z3.FiniteDomain.mk_sort_s z3ctx "expr" (Int64.of_int !sort_size)
    in
    let binop_sort =
      Z3.FiniteDomain.mk_sort_s z3ctx "binop" (Int64.of_int 25)
    in
    ...
    in
    let salloc =
      Z3.FuncDecl.mk_func_decl_s z3ctx "SAlloc" [ expr; str_literal ]
        boolean_sort
    in
    let lval_exp =
    ...
    in
    let memory =
      Z3.FuncDecl.mk_func_decl_s z3ctx "Memory" [ node; loc; value ]
        boolean_sort
    in
    let arrval =
    ...
    in
    let reach =
      Z3.FuncDecl.mk_func_decl_s z3ctx "Reach" [ node ] boolean_sort
    in
    let ioerror =
...
type t = {
  ...
  true_alarm : string;
  buggy_ast : Cil.file;
  sym_diff : SymDiff.SDiff.t list;
  buggy_cfg : (string * string list * string) list;
}
...
let init z3env buggy_src buggy_snk buggy_dir patch_dir out_dir true_alarm
    buggy_ast buggy_cfg =
  {
    z3env;
    fact_files;
...
let set_buggy_src s global = { global with buggy_src = s }
let set_buggy_snk s global = { global with buggy_snk = s }
